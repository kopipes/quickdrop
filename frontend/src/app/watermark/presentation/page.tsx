...
export default function WatermarkPresentationPage() {
  const { state, run, reset } = useProcess('watermark_presentation');
  ...
  return (
    <PageLayout title="Watermark Presentation" description="Add a text or image watermark to every slide.">
      <div className="mx-auto max-w-lg space-y-5">
        {!docFile ? (
          <DropZone accept=".pptx" onFiles={(f) => f[0] && setDocFile(f[0])} maxSize={MAX_PPTX} />
        ) : (
          <div className="space-y-4 rounded-2xl border border-neutral-200/80 bg-white p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setApplyTo('all')} className={`flex-1 rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${applyTo === 'all' ? 'border-primary/40 bg-primary/5 text-primary' : 'border-neutral-200 bg-white text-neutral-700 hover:border-neutral-300'}`}>All Slides</button>
                <button onClick={() => setApplyTo('selected')} className={`flex-1 rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${applyTo === 'selected' ? 'border-primary/40 bg-primary/5 text-primary' : 'border-neutral-200 bg-white text-neutral-700 hover:border-neutral-300'}`}>Selected Slides</button>
              </div>
              {applyTo === 'selected' && (
                <input value={slides} onChange={(e) => setSlides(e.target.value)} placeholder="e.g. 1, 3, 5" className="mt-1 w-full rounded-xl border border-neutral-200 px-3 py-2 text-sm outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20" />
              )}
            </div>
